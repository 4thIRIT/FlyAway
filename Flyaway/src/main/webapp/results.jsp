...
<table class="table">
	...
	<c:forEach var="flights" items="${requestScope.flights}">
		<tr>
			<td>${ flights.getFlightid()}</td>
			<td>${ flights.getFlightdate()}</td>
			<td>${ flights.getAirline()}</td>
			<td>${ flights.getDestination()}</td>
			<td>${ flights.getPrice()}</td>
			<td> <a class="btn btn-primary" href="Purchase?flightid=${ flights.getFlightid()}" role="button">Book Flight</a></td>
		</tr>
	</c:forEach>
